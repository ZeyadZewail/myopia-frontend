...
const DoctorRating: FC<doctorRatingInterface> = ({ patient, doctorRatingData }) => {
	...
	useEffect(() => {
		if (doctorRatingData.length > 0) {
			reset({
				risk_rating: doctorRatingData[0].risk_rating,
	...
	return (
		<Box component="form" sx={{ display: "flex", flexDirection: "column", gap: "inherit" }}>
			...
			<Box sx={{ display: "flex", gap: 2, flexDirection: "column" }}>
				<Box sx={{ display: "flex", gap: 2, justifyContent: "space-between", width: "25%" }}>
					<InputLabel sx={{ height: "fit-content", alignSelf: "center" }}>Risk rating:</InputLabel>
					<Controller
						control={control}
						name={"risk_rating"}
